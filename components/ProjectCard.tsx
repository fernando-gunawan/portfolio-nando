            ...
            >
              {project.visualConfig.data.values.map((val, i) => (
                <div key={i} className="relative group/bar flex flex-col items-center gap-2 w-full h-full justify-end">
                  ...
                  {/* Label */}
                  <span className="text-[10px] text-slate-200 font-mono text-center truncate w-full uppercase tracking-wider">{project.visualConfig.data?.labels[i]}</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="p-6 flex flex-col flex-grow bg-gradient-to-b from-[#0b1121] to-[#020617]">
          ...
          <div className="flex flex-wrap gap-2 mb-6 mt-auto">
            {project.tags.slice(0, 4).map((tag, idx) => (
              <span
                key={idx}
                className="px-3 py-1 bg-white/5 text-xs font-mono font-medium text-slate-100 rounded-full border border-white/5 group-hover:border-accent/30 group-hover:text-accent/90 transition-colors"
              >
                #{tag}
              </span>
            ))}
